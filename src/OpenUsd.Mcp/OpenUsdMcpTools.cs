...
internal static class OpenUsdMcpDescriptions
{
    internal const string OpenScene =
        "Open a USD file beneath the configured read-only source root and create the single isolated overlay session. " +
        ...
        "Errors: invalid_argument for malformed input or a second session; path_denied for unavailable, rooted, escaping, or indirect paths; native_failure when OpenUSD cannot open the stage. " +
    ...
    internal const string CloseScene =
        ...
        "Effects: persists the close journal, releases retained render sources and native stage state, and permits a later open_scene call. " +
        "Result bounds: one structured close acknowledgement and one text summary of at most 2048 characters. " +
        "Errors: no_session when none is active; stale_session for a foreign sessionId; stale_revision for mismatched generation or stageRevision; invalid_argument for malformed values; native_failure for teardown or persistence failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<open_scene sessionId>\",\"generation\":0,\"stageRevision\":1}}.";

    internal const string GetScene =
        "Return the bounded identity and optimistic revision of the active scene without exposing native handles or absolute output paths. " +
        "Preconditions: sessionId, generation, and stageRevision exactly match the active session. " +
        "Effects: read-only; refreshes the native stage revision before comparison. " +
        "Result bounds: one structured session object and one text summary of at most 2048 characters. " +
        "Errors: no_session, stale_session, stale_revision, invalid_argument, or native_failure with their documented meanings. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<open_scene sessionId>\",\"generation\":0,\"stageRevision\":1}}.";
    ...
    internal const string InspectScene =
        ...
        "Preconditions: sessionId, generation, and stageRevision exactly match the active session. " +
        "Effects: read-only; performs count-only native preflight before bounded composed-stage traversal and computes prim, mesh, curve, vertex, face, root, leaf, and depth counts. " +
        "Result bounds: one fixed-shape structured object with scalar counts and one text summary of at most 2048 characters; no arbitrary USDA, layer contents, native handles, or unrestricted paths. " +
        "Errors: quota_exceeded when inspection traversal, retained-path, or geometry budgets are exceeded; no_session, stale_session, stale_revision, invalid_argument, or native_failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<open_scene sessionId>\",\"generation\":0,\"stageRevision\":1}}.";

    internal const string ApplyEdits =
        "Atomically apply typed opinions to the session overlay; arbitrary USDA and executable code are not accepted. " +
        "Preconditions: exact active revision and 1-128 valid edits. Supported kinds are define_prim, set_active, set_double, and clear_overlay_attribute with the companion fields described by the schema. " +
        "Effects: creates a recovery checkpoint, commits all edits or restores all of them, then advances generation and stageRevision on success. " +
        "Result bounds: one fixed-shape commit object and one text summary of at most 2048 characters. " +
        "Errors: invalid_argument for an unsupported or malformed edit; no_session, stale_session, stale_revision, or native_failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<id>\",\"generation\":0,\"stageRevision\":1,\"edits\":[{\"kind\":\"define_prim\",\"primPath\":\"/World/Cube\",\"typeName\":\"Cube\"}]}}.";
    ...
    internal const string CheckpointScene =
        ...
        "Preconditions: sessionId, generation, and stageRevision exactly match the active session. " +
        "Effects: writes one session-confined checkpoint and journal entry; generation and stageRevision remain unchanged. " +
    ...
    internal const string RollbackScene =
        ...
        "Effects: first creates a recovery checkpoint, restores the requested overlay atomically, reloads the stage, and advances generation and stageRevision. " +
        "Result bounds: one fixed-shape rollback object and one text summary of at most 2048 characters. " +
        "Errors: invalid_argument for an unknown or malformed checkpoint; no_session, stale_session, stale_revision, or native_failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<id>\",\"generation\":1,\"stageRevision\":2,\"checkpointId\":\"<checkpointId>\"}}.";
    ...
    internal const string RenderPreview =
        ...
        "Preconditions: exact active revision; dimensions are 1-4096; views contain 1-16 finite time codes; still has exactly one view. " +
        "Effects: read-only scene access but consumes bounded render and artifact quotas. Inline image blocks are emitted only at or below the configured byte cap; otherwise resource links are emitted. " +
        "Result bounds: at most 16 artifact descriptors plus one text block and at most 16 image/resource-link blocks. " +
        "Errors: invalid_argument, no_session, stale_session, stale_revision, quota_exceeded, or render_failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<id>\",\"generation\":0,\"stageRevision\":1,\"kind\":\"still\",\"width\":1024,\"height\":1024,\"views\":[{\"name\":\"hero\",\"timeCode\":0}]}}.";

    internal const string AnalyzeScene =
        "Run deterministic camera, lighting, render-setting, performance, composition, and validation analyzers over bounded detached observations. " +
        "Preconditions: exact active revision and finite observations satisfying all schema ranges and cross-field rules. " +
        "Effects: read-only; replaces the prior proposal set and binds every new proposal to this generation and stageRevision. " +
        "Result bounds: at most 128 deterministically ordered proposal objects and one text summary of at most 2048 characters. " +
        "Errors: invalid_argument, no_session, stale_session, stale_revision, or native_failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<id>\",\"generation\":0,\"stageRevision\":1,\"observations\":{\"rendererId\":\"silk\",\"qualityPreset\":\"balanced\"}}}. Omitted observation fields use schema defaults.";

    internal const string ApplyProposals =
        "Atomically apply selected overlay-applicable proposals from the latest analysis. " +
        "Preconditions: exact active revision; 1-128 proposal IDs exist in the latest analyze_scene result, remain revision-current, and are all overlay_applicable. " +
        "Effects: creates a recovery checkpoint, commits the derived typed edits, advances generation and stageRevision, and invalidates the consumed proposal set. " +
        "Result bounds: at most 128 sorted applied IDs in one fixed-shape object and one text summary of at most 2048 characters. " +
        "Errors: proposal_stale for revision-stale, diagnostic_only, or flatten_only proposals; quota_exceeded for history or workspace limits; invalid_argument for malformed or unknown IDs; no_session, stale_session, stale_revision, or native_failure. " +
        "Example arguments: {\"request\":{\"sessionId\":\"<id>\",\"generation\":0,\"stageRevision\":1,\"proposalIds\":[\"<analyze_scene proposal id>\"]}}.";

    internal const string FinalizeScene =
        "Persist the overlay, export a flattened final stage, write deterministic JSON/Markdown reports and a manifest, and publish report resources. " +
        "Preconditions: sessionId, generation, and stageRevision exactly match the active session. Preview artifacts are optional; missing previews are recorded as partial failures rather than invented. " +
        "Effects: writes only inside the session output directory and keeps the session active. " +
        "Result bounds: at most three report-resource descriptors, at most 16 bounded failure messages, at most 16 additional content blocks, and one text summary of at most 2048 characters. " +
        "Errors: invalid_argument, no_session, stale_session, stale_revision, or native_failure. Per-artifact export or publication failures may instead return partial=true with bounded failures. " +
    ...
    internal const string PresentScene =
        ...
        "Preconditions: exact active revision; finalize_scene produced a final stage for that same revision; renderer is auto, silk, or storm; optional cameraPath is an absolute USD prim path. " +
        "Effects: starts one child process with an argument list and redirected protocol-sensitive streams; never executes a caller-supplied executable or unrestricted filesystem path. " +
        ...
        "Example arguments: {\"request\":{\"sessionId\":\"<id>\",\"generation\":1,\"stageRevision\":2,\"renderer\":\"auto\",\"cameraPath\":\"/World/Camera\"}}.";

    internal const string ArtifactResource =
        "Read one immutable artifact previously returned by an OpenUSD MCP tool. " +
        "Preconditions: use an exact openusd://artifact/{id} URI from a tool result; id is percent-decoded, contains no path separators or control characters, and is at most 1024 characters. " +
        "Result: textual MIME types return UTF-8 text content; other MIME types return base64 MCP blob content with the stored MIME type. Content size is bounded by the configured artifact-store quota and cannot be modified through MCP. " +
        "Errors: invalid_argument for an invalid id, artifact_not_found for an unknown or expired process-local artifact, and artifact_invalid_text when a textual artifact is not valid UTF-8. " +
    ...
    OpenUsdMcpProtocolOptions protocolOptions)
{
    ...
    public ValueTask<CallToolResult> OpenSceneAsync(
        [Description("Request object containing the confined sourcePath. See nested schema descriptions and the tool example.")] OpenSceneRequest request,
    ...
    public ValueTask<CallToolResult> CloseSceneAsync(
        [Description("Exact optimistic revision returned by the preceding successful scene tool.")] SceneRevisionRequest request,
        CancellationToken cancellationToken) =>
    ...
    public ValueTask<CallToolResult> ApplyEditsAsync(
        [Description("Exact optimistic revision plus an atomic list of 1-128 typed edits. See nested kind-specific field semantics.")] ApplyEditsRequest request,
        CancellationToken cancellationToken) =>
    ...
    public ValueTask<CallToolResult> RollbackSceneAsync(
        [Description("Exact optimistic revision and an existing checkpointId from this active session.")] RollbackSceneRequest request,
        CancellationToken cancellationToken) =>
    ...
    public ValueTask<CallToolResult> RenderPreviewAsync(
        [Description("Exact optimistic revision, capture mode, bounded dimensions, and 1-16 ordered views.")] RenderPreviewRequest request,
        CancellationToken cancellationToken) =>
    ...
    public ValueTask<CallToolResult> AnalyzeSceneAsync(
        [Description("Exact optimistic revision and bounded finite detached technical observations.")] AnalyzeSceneRequest request,
        CancellationToken cancellationToken) =>
    ...
    public ValueTask<CallToolResult> ApplyProposalsAsync(
        [Description("Exact optimistic revision and 1-128 IDs from the latest analyze_scene result.")] ApplyProposalsRequest request,
        CancellationToken cancellationToken) =>
    ...
    public ValueTask<CallToolResult> PresentSceneAsync(
        [Description("Exact optimistic revision plus constrained renderer and optional USD camera prim path.")] PresentSceneRequest request,
        CancellationToken cancellationToken) =>
