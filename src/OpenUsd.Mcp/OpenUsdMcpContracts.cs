...
internal sealed class OpenSceneRequest
{
    ...
    [Description("Relative USD file path under the configured read-only source root, for example \"assets/robot.usda\". Supported extensions: .usd, .usda, .usdc, and .usdz. Absolute paths, traversal outside the root, control characters, and paths longer than 1024 characters are rejected.")]
...
internal sealed class SceneRevisionRequest
{
    ...
    [Description("Exact active session identifier returned by open_scene; 1-128 non-control characters. A different identifier produces stale_session.")]
    public required string SessionId { get; init; }

    [JsonPropertyName("generation")]
    [Range(0, long.MaxValue)]
    [Description("Exact non-negative transactional generation returned by the preceding scene-mutating result. An older or newer value produces stale_revision.")]
    public long Generation { get; init; }

    [JsonPropertyName("stageRevision")]
    [Description("Exact unsigned native stage revision returned by the preceding result. A mismatched value produces stale_revision.")]
...
internal sealed class ApplyEditsRequest : SceneRevisionRequestBase
{
    ...
    [Description("Atomic list of 1-128 typed overlay edits. The whole batch commits or rolls back. Each element uses kind-specific fields; arbitrary USDA and code are not accepted.")]
...
internal sealed class RollbackSceneRequest : SceneRevisionRequestBase
{
    ...
    [Required]
    [MaxLength(OpenUsdMcpLimits.MaximumIdentifierLength)]
    [Description("Existing checkpoint identifier returned by checkpoint_scene or apply_edits; 1-128 non-control characters.")]
...
internal sealed class RenderPreviewRequest : SceneRevisionRequestBase
{
    ...
    [Description("Capture mode. still requires exactly one view; contact_sheet combines all views into one image; turntable emits one image per view.")]
    public required string Kind { get; init; }

    [JsonPropertyName("width")]
    [Range(1, 4096)]
    [DefaultValue(1024)]
    [Description("Output width in pixels, inclusive range 1-4096. Contact-sheet width is divided among its view columns.")]
    public int Width { get; init; } = 1024;

    [JsonPropertyName("height")]
    [Range(1, 4096)]
    [DefaultValue(1024)]
    [Description("Output height in pixels, inclusive range 1-4096. Contact-sheet height is divided among its view rows.")]
    ...
    [Description("List of 1-16 automatic-camera views. still requires one item; contact_sheet and turntable preserve list order.")]
...
internal sealed class AnalyzeSceneRequest : SceneRevisionRequestBase
{
    ...
    [Required]
    [Description("Detached, bounded technical observations used by deterministic analyzers. These are metrics only; USDA text, source code, and native handles are not accepted.")]
...
internal sealed class ApplyProposalsRequest : SceneRevisionRequestBase
{
    ...
    [Description("List of 1-128 distinct proposal IDs from the latest analyze_scene result. Every selected proposal must be overlay_applicable and match the current generation and stage revision.")]
...
internal sealed class PresentSceneRequest : SceneRevisionRequestBase
{
    ...
    [MaxLength(OpenUsdMcpLimits.MaximumPathLength)]
    [Description("Optional absolute USD camera prim path such as \"/World/Camera\". This is a prim path, never a filesystem path, and must contain valid USD identifiers.")]
...
internal sealed class WorkspaceEditDto
{
    ...
    [Description("Typed edit discriminator. Required companion fields: define_prim may use typeName; set_active requires active; set_double requires attributeName and value and may use timeCode; clear_overlay_attribute requires attributeName.")]
    public required string Kind { get; init; }
    ...
    [MaxLength(OpenUsdMcpLimits.MaximumIdentifierLength)]
    [Description("Optional USD type identifier for define_prim, for example \"Xform\" or \"Mesh\". Omit for other kinds. Maximum 128 characters and valid USD identifier syntax.")]
    public string? TypeName { get; init; }

    [JsonPropertyName("active")]
    [Description("Required only for set_active. true authors activation; false authors deactivation in the overlay. Omit for other kinds.")]
    public bool? Active { get; init; }

    [JsonPropertyName("attributeName")]
    [MaxLength(OpenUsdMcpLimits.MaximumIdentifierLength)]
    [Description("Required for set_double and clear_overlay_attribute. USD property name such as \"xformOp:translateX\"; maximum 128 characters. Omit for other kinds.")]
    public string? AttributeName { get; init; }

    [JsonPropertyName("value")]
    [Description("Required finite numeric value for set_double. NaN and infinities are rejected. Omit for other kinds.")]
    public double? Value { get; init; }

    [JsonPropertyName("timeCode")]
    [Description("Optional finite time code for set_double. Omit to author the default value; omit for other kinds.")]
...
internal sealed class CaptureViewDto
{
    ...
    [Required]
    [MaxLength(OpenUsdMcpLimits.MaximumIdentifierLength)]
    [Description("Display name used in generated artifact identifiers; 1-128 non-control characters. Unsafe filename characters are replaced, not interpreted as paths.")]
...
internal sealed class AnalysisObservationsDto
{
    ...
    [Description("Zero to 128 validation messages, each at most 4096 non-control characters. Supply concise findings only; USDA text and source code are rejected by contract.")]
    public IReadOnlyList<string> ValidationIssues { get; init; } = [];
}

internal sealed record McpSessionDto(
    [property: JsonPropertyName("sessionId")]
    [property: Description("Opaque identifier for the one active session; pass unchanged to every revision-bound tool.")]
    string SessionId,
    [property: JsonPropertyName("generation")]
    [property: Description("Current transactional generation; pass unchanged until a mutating tool returns a successor value.")]
    long Generation,
    ...
    [property: Description("Original source path relative to the configured source root; never an unrestricted absolute path.")]
    string SourcePath,
...
internal sealed record McpSceneInspectionDto(
    ...
    [property: Description("Absolute USD path of the stage default prim, or an empty string when no default prim is authored.")]
...
internal sealed record McpArtifactDto(
    ...
    [property: Description("True when the artifact is eligible for an inline image content block; false means clients should read the URI.")]
    bool Inline);
...
internal sealed record McpCaptureResultDto(
    ...
    [property: Description("Ordered immutable PNG descriptors; at most 16. Tool content includes at most 16 corresponding image or resource-link blocks.")]
...
internal sealed record McpProposalDto(
    ...
    [property: Description("Analyzer category: camera, lighting, rendersettings, performance, composition, or validation.")]
    string Category,
    ...
    [property: Description("overlay_applicable, flatten_only, or diagnostic_only. Only overlay_applicable IDs may be passed to apply_proposals.")]
...
internal sealed record McpFinalizationResultDto(
    ...
    [property: Description("True when one or more requested finalization artifacts failed while the bounded report set was still produced.")]
    bool Partial,
...
internal sealed record McpToolErrorEnvelope(
    ...
    [property: Description("Deterministic tool-execution error. Tool errors set isError=true and do not satisfy the success output schema.")]
    McpToolErrorDto Error);

internal sealed record McpToolErrorDto(
    [property: JsonPropertyName("code")]
    [property: Description("Stable code: invalid_argument, path_denied, no_session, stale_session, stale_revision, proposal_stale, quota_exceeded, native_failure, render_failure, or launch_failure.")]
    string Code,
