...
internal static class UsdRenderSchema
{
    internal static bool TryValidate(UsdPrim prim, OpenUsdNativeRenderSchemaKind schemaKind, out UsdStage? stage)
    {
        stage = null;
        ...
        try
        {
            ...
            return stage.Native.IsRenderSchema(prim.Path, schemaKind);
        }
    ...
    internal static UsdStage Validate(UsdPrim prim, OpenUsdNativeRenderSchemaKind schemaKind, string schemaName)
    {
        if (string.IsNullOrWhiteSpace(prim.Path))
        {
        ...
        if (!stage.Native.IsRenderSchema(prim.Path, schemaKind))
        {
