...
internal static class UsdVolSchema
{
    internal static bool TryValidate(UsdPrim prim, OpenUsdNativeVolSchemaKind schemaKind, out UsdStage? stage)
    {
        stage = null;
        ...
        try
        {
            ...
            return stage.Native.IsVolSchema(prim.Path, schemaKind);
        }
    ...
    internal static UsdStage Validate(UsdPrim prim, OpenUsdNativeVolSchemaKind schemaKind, string schemaName)
    {
        if (string.IsNullOrWhiteSpace(prim.Path))
        {
        ...
        if (!stage.Native.IsVolSchema(prim.Path, schemaKind))
        {
