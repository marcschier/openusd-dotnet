...
public sealed class NativeAbiVersionTests
{
    ...
    public async Task StatusExportsAreGuardedAndInitializeFailureOutputs()
    {
        ...
        await Assert.That(declarations.Count).IsEqualTo(217);
        await Assert.That(definitions.Count).IsEqualTo(218);
        await Assert.That(outputBearingExports.Count).IsEqualTo(107);
        await Assert.That(
            Regex.Count(
                implementation,
                @"// ABI_OUTPUT_INITIALIZATION",
                RegexOptions.CultureInvariant)).IsEqualTo(107);
        await Assert.That(
            Regex.Count(
                implementation,
                @"\breturn GuardStage\(stage, error",
                RegexOptions.CultureInvariant)).IsEqualTo(202);
