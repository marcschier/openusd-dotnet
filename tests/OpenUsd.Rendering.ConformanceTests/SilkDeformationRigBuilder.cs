...
/// </summary>
/// <remarks>
/// The rig is deliberately not constructed directly. Encoding and decoding
/// means a conformance case exercises the same bytes, the same bounds checks
/// and the same identity recomputation a real page goes through, so a gate
/// cannot pass against a rig shape the parser would have refused.
/// </remarks>
internal static class SilkDeformationRigBuilder
{
        ...
        bool includeBindNormals = true)
    {
        ...
        SilkDeformationOptions options = includeBindNormals
