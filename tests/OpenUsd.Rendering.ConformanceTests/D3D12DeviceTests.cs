...
public sealed class D3D12DeviceTests
{
    ...
    public async Task UniversalMaterialUsesBoundedRootDescriptorTables()
    {
        ...
        await Assert.That(plan.SampledTextureCount).IsEqualTo(8u);
        await Assert.That(plan.SamplerCount).IsEqualTo(8u);
        ...
        await Assert.That(plan.GetDescriptorOffset(
            ...
            SilkBindingKind.SampledTexture)).IsEqualTo(7u);
        await Assert.That(plan.GetDescriptorOffset(
            0,
    ...
    [Test]
    public async Task WarpRoundTripsFloatingPointTextures()
    {
        if (!OperatingSystem.IsWindows())
        {
            Skip.Test("This test is only applicable on Windows.");
    ...
    [Test]
    public async Task WarpUploadsMultiLevelMipChainAndPreservesBaseLevelReadback()
    {
        if (!OperatingSystem.IsWindows())
        {
            Skip.Test("This test is only applicable on Windows.");
