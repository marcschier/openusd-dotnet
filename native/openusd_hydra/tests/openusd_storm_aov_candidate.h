...
#ifndef OPENUSD_STORM_AOV_CANDIDATE_H
#define OPENUSD_STORM_AOV_CANDIDATE_H

#include "openusd_hydra.h"

/* Private executable contract. Not installed, advertised, or part of ABI 8. */
#define OPENUSD_STORM_AOV_CANDIDATE_VERSION 1u
...
/* All outputs belong to one completed render. Failure initializes *owner to
 * NULL. Capture requires the renderer's creation thread and original context.
 * Only one unreleased owner per renderer is admitted. */
OPENUSD_HYDRA_API openusd_status openusd_storm_aov_candidate_capture(
    openusd_storm_renderer* renderer,
    const openusd_storm_aov_request* request,
    openusd_storm_aov_owner** owner,
    openusd_error_buffer* error);

/* Initialize struct_size/version before calling. Failure clears the complete
 * view. Pointers remain valid until release, including after renderer teardown.
 * Paths are length-delimited UTF-8, not NUL-terminated. Rows are top-down.
 * Background identity indices are UINT32_MAX; unresolved IDs have their own
 * explicit table record. Neither Hydra IDs nor table indices are authored IDs. */
OPENUSD_HYDRA_API openusd_status openusd_storm_aov_candidate_get_view(
    const openusd_storm_aov_owner* owner,
    openusd_storm_aov_view* view,
    openusd_error_buffer* error);

/* CPU-only, NULL-safe, callable on any thread without a GL context. */
OPENUSD_HYDRA_API void openusd_storm_aov_candidate_release(
