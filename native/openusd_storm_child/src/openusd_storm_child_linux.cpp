...
openusd_status CaptureFramebuffer(ChildState* child, Command* command)
{
    ...
    if (command->aovs != nullptr &&
        (width != command->aovs->request.width || height != command->aovs->request.height))
    {
        command->error = "The preserved Storm frame does not match the admitted AOV raster.";
        return OPENUSD_STATUS_INVALID_ARGUMENT;
    }
...
openusd_status CaptureAovs(ChildState* child, Command* command)
{
    ...
    const openusd_status status = Guard(&native_error, [&]
    {
        ...
        capture_status = command->aovs->Execute(child->renderer, 0, &native_error);
        if (capture_status != OPENUSD_STATUS_OK)
        {
            return capture_status;
        }
