...
openusd_status ExecuteTransformOverrides(
    ChildState* child,
    OpenUsdStormChildTransformOverridePayload* transform_overrides,
    std::string& error)
{
    std::lock_guard context_lock(child->context_gate);
    const openusd_status status = EnsureContextReadyLocked(child, error);
    ...
    CurrentContextScope current_context(child->context);
    return transform_overrides->Execute(child->renderer, error);
}

openusd_status ExecuteDeformationOverrides(
    ChildState* child,
    OpenUsdStormChildDeformationOverridePayload* deformation_overrides,
    std::string& error)
{
    std::lock_guard context_lock(child->context_gate);
    const openusd_status status = EnsureContextReadyLocked(child, error);
    ...
    CurrentContextScope current_context(child->context);
    return deformation_overrides->Execute(child->renderer, error);
}
        ...
        while (!stop)
        {
            ...
            switch (command->kind)
            {
                ...
                case CommandKind::TransformOverrides:
                    command->status = ExecuteTransformOverrides(
                        child,
                        command->transform_overrides.get(),
                        command->error);
                    break;
                case CommandKind::DeformationOverrides:
                    command->status = ExecuteDeformationOverrides(
                        child,
                        command->deformation_overrides.get(),
                        command->error);
                    break;
...
openusd_status ValidateChild(
                ...
                openusd_error_buffer* error)
            {
                if (update == nullptr ||
                    ...
                    update->version !=
                        OPENUSD_STORM_TRANSFORM_OVERRIDE_UPDATE_VERSION ||
                     ...
                     update->path_bytes == nullptr))
                {
                    WriteError(
                        error,
                        "The packed Storm child transform override update is "
                        "invalid.");
                    return OPENUSD_STATUS_INVALID_ARGUMENT;
                }
                         ...
                         OPENUSD_STORM_TRANSFORM_OVERRIDE_DIAGNOSTICS_VERSION))
                {
                    WriteError(
                        error,
                        "The Storm child transform override diagnostics struct "
                        "is invalid.");
                    return OPENUSD_STATUS_INVALID_ARGUMENT;
                }
                const openusd_status child_status = ValidateChild(child, error);
                ...
                command->wait = true;
                command->transform_overrides = std::make_unique<
                    OpenUsdStormChildTransformOverridePayload>();
                command->transform_overrides->update = *update;
                command->transform_overrides->capture_diagnostics =
                    diagnostics != nullptr;
                ...
                openusd_error_buffer* error)
            {
                if (update == nullptr ||
                    ...
                    update->version !=
                        OPENUSD_STORM_DEFORMATION_OVERRIDE_UPDATE_VERSION ||
                     ...
                     update->path_bytes == nullptr))
                {
                    WriteError(
                        error,
                        "The packed Storm child deformation override update is "
                        "invalid.");
                    return OPENUSD_STATUS_INVALID_ARGUMENT;
                }
                if (diagnostics != nullptr &&
                         ...
                         OPENUSD_STORM_DEFORMATION_OVERRIDE_DIAGNOSTICS_VERSION))
                {
                    WriteError(
                        error,
                        "The Storm child deformation override diagnostics "
                        "struct is invalid.");
                    return OPENUSD_STATUS_INVALID_ARGUMENT;
                }
                const openusd_status child_status = ValidateChild(child, error);
                ...
                command->wait = true;
                command->deformation_overrides = std::make_unique<
                    OpenUsdStormChildDeformationOverridePayload>();
                command->deformation_overrides->update = *update;
                command->deformation_overrides->capture_diagnostics =
                    diagnostics != nullptr;
                if (update->item_count != 0)
                {
                ...
                if (update->point_count != 0)
                {
                    command->deformation_overrides->points.assign(
                        update->points,
                        update->points +
                            (static_cast<size_t>(update->point_count) * 3u));
                }
                if (update->path_bytes_size != 0)
                {
                ...
                openusd_error_buffer* error)
            {
                ...
                {
                    const std::shared_ptr<ChildState> state = LookupChild(child);
                    return QueueTransformOverrides(
                        state.get(),
                        update,
                        diagnostics,
                        error);
                ...
                openusd_error_buffer* error)
            {
                ...
                {
                    const std::shared_ptr<ChildState> state = LookupChild(child);
                    return QueueDeformationOverrides(
                        state.get(),
                        update,
                        diagnostics,
                        error);
