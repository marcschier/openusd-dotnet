...
[[fragment]] pixelOutput_0 fragmentMain_uv_normal(pixelInput_0 _S1 [[stage_in]], float4 position_0 [[position]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], sampler materialSampler_1 [[sampler(0)]], SLANG_ParameterGroup_SceneParameters_0 constant* SceneParameters_1 [[buffer(0)]])
{
    ...
    pixelOutput_0 _S2 = { float4(mix(abs(_S1.normal_0), abs(((normalTexture_1).sample((materialSampler_1), (_S1.texCoord_0))).xyz * float3(2.0f)  - float3(1.0f) ), float3(0.25f) ), 1.0f) * SceneParameters_1->tint_0 };
    return _S2;
}
