...
[[fragment]] pixelOutput_0 fragmentMain_uv_normal(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], sampler materialSampler_1 [[sampler(0)]])
{
    ...
    for(;;)
    {
        ...
        {
            _S36 = (float4((&_S3->lightColorIntensity_0)->data_1[lightIndex_0]) ).w;
        }
        bool _S37;
        if(_S33)
        {
        ...
        {
            _S39 = (float4((&_S3->lightControls_0)->data_1[lightIndex_0]) ).x;
        }
        bool _S40;
        if(_S33)
        {
        ...
        {
            _S41 = (float4((&_S3->lightControls_0)->data_1[lightIndex_0]) ).y;
        }
        bool _S42;
        if(_S33)
        {
        ...
        for(;;)
        {
            ...
            if(lightType_0 >= 2.0f)
            {
                float3 toLight_0 = (float4((&_S3->lightPositionType_0)->data_1[lightIndex_0]) ).xyz + sampleOffsets_0[sampleIndex_0] - _S30;
