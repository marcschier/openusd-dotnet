...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_normal_roughmetal_emissive(pixelInput_0 _S1 [[stage_in]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], texture2d<float, access::sample> roughnessMetallicTexture_1 [[texture(2)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]])
{
    ...
    float roughness_0 = clamp(clamp(_S4.y, 0.00999999977648258f, 1.0f) * sampledRoughnessMetallic_0.y, 0.00999999977648258f, 1.0f);
    float metallic_0 = saturate(saturate(_S4.x) * sampledRoughnessMetallic_0.z);
    ...
    pixelOutput_0 _S23 = { float4(float3((occlusion_0 * normalDotLight_0))  * (diffuse_3 + specular_0) * (_S22.xyz * float3(_S7.w)  * _S13) + diffuseColor_1 * float3(_S22.w)  + emissiveColor_0, opacity_1) };
    return _S23;
}
