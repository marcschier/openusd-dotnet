...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_normal(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]])
{
    ...
    float3 sampledNormal_0 = (((&kernelContext_0)->normalTexture_0).sample(((&kernelContext_0)->materialSampler_0), (_S1.texCoord_0))).xyz * float3(2.0f)  - _S9;
    ...
    for(;;)
    {
        ...
        if(_S30)
        {
            _S31 = normalize((float4(surface_0.lightDirectionIntensity_0) ).xyz);
        }
        else
        {
            _S31 = normalize((float4((&_S2->lightDirectionRadius_0)->data_2[lightIndex_0]) ).xyz);
        }
        bool _S32;
        if(_S29)
        {
        ...
        if(lightType_0 >= 2.0f)
        {
            float3 toLight_0 = (float4((&_S2->lightPositionType_0)->data_2[lightIndex_0]) ).xyz - _S26;
            float _S37 = max(dot(toLight_0, toLight_0), 0.00100000004749745f);
            float lightIntensity_2 = lightIntensity_0 / _S37;
            lightDirection_0 = toLight_0 * float3(rsqrt(_S37)) ;
            lightIntensity_1 = lightIntensity_2;
        }
        else
        {
            lightDirection_0 = _S31;
        ...
        float3 _S38 = float3(pow(max(0.0f, 1.0f - saturate(dot(lightEye_0, half_0))), 5.0f)) ;
        float3 _S39 = mix(normalIncidence_0, grazingIncidence_0, _S38);
        float3 directDiffuse_0 = diffuse_1 * (_S9 - _S39);
        float alpha_0 = _S12 * _S12;
        float alphaSquared_0 = alpha_0 * alpha_0;
        float _S40 = normalDotHalf_0 * normalDotHalf_0;
        float denominator_0 = _S40 * (alphaSquared_0 - 1.0f) + 1.0f;
        float k_0 = alpha_0 * 0.5f;
        float _S41 = 1.0f - k_0;
        float3 _S42 = float3((4.0f * normalDotLight_0 * _S11 + 0.00100000004749745f)) ;
        float3 _S43 = _S39 * float3((_S11 / (_S11 * _S41 + k_0) * (normalDotLight_0 / (normalDotLight_0 * _S41 + k_0))))  * float3(((alphaSquared_0 + 0.00100000004749745f) / (denominator_0 * denominator_0 * 3.14159274101257324f)))  / _S42;
        float3 directSpecular_0;
        if(_S13 > 0.0f)
        {
            float alpha_1 = _S14 * _S14;
            float alphaSquared_1 = alpha_1 * alpha_1;
            float denominator_1 = _S40 * (alphaSquared_1 - 1.0f) + 1.0f;
            float k_1 = alpha_1 * 0.5f;
            float _S44 = 1.0f - k_1;
            directSpecular_0 = _S43 + float3(_S13)  * (mix(float3((reflectanceRatio_0 * reflectanceRatio_0)) , float3(1.0f, 1.0f, 1.0f), _S38) * float3((_S11 / (_S11 * _S44 + k_1) * (normalDotLight_0 / (normalDotLight_0 * _S44 + k_1))))  * float3(((alphaSquared_1 + 0.00100000004749745f) / (denominator_1 * denominator_1 * 3.14159274101257324f)))  / _S42);
        }
        else
        {
            directSpecular_0 = _S43;
        }
        float3 _S45 = lightColor_0 * float3(lightIntensity_1) ;
        float3 color_2 = color_1 + float3((_S7 * normalDotLight_0))  * (directDiffuse_0 * float3(diffuseScale_0)  * (_S45 * _S17) + directSpecular_0 * float3(specularScale_0)  * _S45);
        lightIndex_0 = lightIndex_0 + 1U;
        color_1 = color_2;
    }
    pixelOutput_0 _S46 = { float4(color_1 + emissiveColor_0, opacity_1) };
    return _S46;
}
