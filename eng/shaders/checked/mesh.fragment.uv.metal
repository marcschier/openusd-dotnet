...
[[fragment]] pixelOutput_0 fragmentMain_uv(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> shadowAtlas_1 [[texture(16)]], sampler shadowSampler_1 [[sampler(13)]], texture2d<float, access::sample> environmentBrdf_1 [[texture(19)]], sampler environmentBrdfSampler_1 [[sampler(15)]], texture2d<float, access::sample> environmentIrradiance_1 [[texture(17)]], sampler environmentSampler_1 [[sampler(14)]], texture2d<float, access::sample> environmentSpecular_1 [[texture(18)]])
{
    ...
    for(;;)
    {
        ...
        for(;;)
        {
            ...
            float3 _S74 = _S70 * float3((_S17 / (_S17 * _S72 + k_0) * (normalDotLight_0 / (normalDotLight_0 * _S72 + k_0))))  * float3((alphaSquared_0 / max(scaled_0, 1.00000000317107685e-30f)))  / _S73;
            ...
            if(clearcoatAmount_0 > 0.0f)
            {
                ...
                float _S76 = 1.0f - k_1;
                directSpecular_0 = _S74 + float3(clearcoatAmount_0)  * (mix(float3((reflectanceRatio_0 * reflectanceRatio_0)) , float3(1.0f, 1.0f, 1.0f), _S69) * float3((_S17 / (_S17 * _S76 + k_1) * (normalDotLight_0 / (normalDotLight_0 * _S76 + k_1))))  * float3((alphaSquared_1 / max(scaled_1, 1.00000000317107685e-30f)))  / _S73);
            }
            else
            {
