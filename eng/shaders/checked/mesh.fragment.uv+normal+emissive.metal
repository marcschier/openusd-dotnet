...
[[fragment]] pixelOutput_0 fragmentMain_uv_normal_emissive(pixelInput_0 _S2 [[stage_in]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]])
{
    ...
    pixelOutput_0 _S17 = { float4(float3((occlusion_0 * normalDotLight_2))  * (diffuse_3 + specular_1) * (_S16.xyz * float3(_S8.w)  * _S13) + diffuseColor_0 * float3(_S16.w)  + emissiveColor_0, opacity_0) };
    return _S17;
}
