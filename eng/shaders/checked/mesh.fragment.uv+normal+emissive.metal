...
[[fragment]] pixelOutput_0 fragmentMain_uv_normal_emissive(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]])
{
    ...
    for(;;)
    {
        ...
        {
            _S35 = (float4((&_S3->lightColorIntensity_0)->data_2[lightIndex_0]) ).w;
        }
        bool _S36;
        if(_S32)
        {
        ...
        for(;;)
        {
            ...
            float3 directDiffuse_0 = diffuse_1 * (_S12 - _S52);
            ...
            color_2 = color_2 + float3((_S8 * emissionScale_0 * normalDotLight_0))  * (directDiffuse_0 * float3(_S38)  * (_S58 * _S20) + directSpecular_0 * float3(_S40)  * _S58);
            sampleIndex_0 = sampleIndex_0 + 1U;
        }
    ...
    if((_S2.y) == 1U)
    {
        color_1 = color_3 / (_S12 + max(color_3, float3(0.0f, 0.0f, 0.0f)));
    }
    else
    {
        color_1 = color_3;
