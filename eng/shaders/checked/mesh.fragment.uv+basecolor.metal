...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler baseColorSampler_1 [[sampler(0)]])
{
    ...
    bool _S14 = (uint(round(_S11.w)) & 1U) != 0U;
    for(;;)
    {
        if(!_S14)
        {
    ...
    float _S22 = saturate(abs(dot(normal_2, diffuseColor_0)) + 0.00000999999974738f);
    ...
    float3 diffuse_0 = diffuseColor_1 / _S28;
    ...
    else
    {
        ...
        float3 specularTint_0 = mix(float3(1.0f, 1.0f, 1.0f), diffuseColor_1, _S30);
    ...
    float3 _S36 = normalize((((float4(diffuseColor_0, 0.0f)) * (_S35))).xyz);
    float3 _S37 = (((float4(_S1.eyePosition_0, 1.0f)) * (_S35))).xyz;
    float4 _S38 = float4(surface_0.lightColorAmbient_0) ;
    float3 color_0 = diffuseColor_1 * float3(_S38.w)  + diffuseColor_1 * _S31.xyz;
    ...
    for(;;)
    {
        ...
        {
            _S44 = (float4((&_S4->lightColorIntensity_0)->data_1[lightIndex_0]) ).xyz;
        }
        bool _S45;
        if(_S40)
        {
        ...
        for(;;)
        {
            ...
            float3 _S66 = _S44 * float3(sampleIntensity_1) ;
            color_2 = color_2 + float3((_S10 * emissionScale_0 * normalDotLight_0))  * (directDiffuse_0 * float3(_S46)  * (_S66 * _S28) + directSpecular_0 * float3(_S48)  * _S66);
            sampleIndex_0 = sampleIndex_0 + 1U;
        }
    ...
    pixelOutput_0 _S67 = { float4(color_1, opacity_1) };
    return _S67;
}
