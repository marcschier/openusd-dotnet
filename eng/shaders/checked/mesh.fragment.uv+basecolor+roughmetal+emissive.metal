...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_roughmetal_emissive(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> roughnessMetallicTexture_1 [[texture(2)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]])
{
    ...
    for(;;)
    {
        ...
        if(_S28)
        {
            _S29 = normalize((float4(surface_0.lightDirectionIntensity_0) ).xyz);
        }
        else
        {
            _S29 = normalize((float4((&_S2->lightDirectionRadius_0)->data_2[lightIndex_0]) ).xyz);
        }
        bool _S30;
        if(_S27)
        {
        ...
        float _S39 = 1.0f - k_0;
        float3 _S40 = float3((4.0f * normalDotLight_0 * _S9 + 0.00100000004749745f)) ;
        float3 _S41 = _S37 * float3((_S9 / (_S9 * _S39 + k_0) * (normalDotLight_0 / (normalDotLight_0 * _S39 + k_0))))  * float3(((alphaSquared_0 + 0.00100000004749745f) / (denominator_0 * denominator_0 * 3.14159274101257324f)))  / _S40;
        ...
        if(_S11 > 0.0f)
        {
            ...
            float _S42 = 1.0f - k_1;
            directSpecular_0 = _S41 + float3(_S11)  * (mix(float3((reflectanceRatio_0 * reflectanceRatio_0)) , float3(1.0f, 1.0f, 1.0f), _S36) * float3((_S9 / (_S9 * _S42 + k_1) * (normalDotLight_0 / (normalDotLight_0 * _S42 + k_1))))  * float3(((alphaSquared_1 + 0.00100000004749745f) / (denominator_1 * denominator_1 * 3.14159274101257324f)))  / _S40);
        }
        else
        {
            directSpecular_0 = _S41;
        }
        float3 color_2 = color_1 + float3((_S6 * normalDotLight_0))  * (directDiffuse_0 * float3(diffuseScale_0)  + directSpecular_0 * float3(specularScale_0) ) * (lightColor_0 * float3(lightIntensity_1)  * _S15);
        lightIndex_0 = lightIndex_0 + 1U;
        color_1 = color_2;
    }
    pixelOutput_0 _S43 = { float4(color_1 + emissiveColor_0, opacity_1) };
    return _S43;
}
