...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_roughmetal_emissive(pixelInput_0 _S1 [[stage_in]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> roughnessMetallicTexture_1 [[texture(2)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]])
{
    ...
    {
        _S5 = opacity_1 < opacityThreshold_0;
    }
    else
    {
        _S5 = false;
    ...
    pixelOutput_0 _S22 = { float4(float3((occlusion_0 * normalDotLight_0))  * (diffuse_3 + specular_0) * (_S21.xyz * float3(_S6.w)  * _S12) + diffuseColor_1 * float3(_S21.w)  + emissiveColor_0, opacity_1) };
    return _S22;
}
