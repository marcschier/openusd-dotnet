...
[[fragment]] pixelOutput_0 fragmentMain_uv_normal_metallic_emissive(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], sampler normalSampler_1 [[sampler(1)]], texture2d<float, access::sample> metallicTexture_1 [[texture(4)]], sampler metallicSampler_1 [[sampler(5)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]], sampler emissiveSampler_1 [[sampler(3)]])
{
    ...
    for(;;)
    {
        ...
        texture2d<float, access::sample> _S24 = (&kernelContext_0)->metallicTexture_0;
        ...
        float4 metadata_1 = round((((&kernelContext_0)->metallicTexture_0).read(vec<uint,2>(((_S25)).xy), uint(((_S25)).z))) * float4(255.0f) );
        ...
        if(hasSceneLighting_0)
        {
            ...
            _S3 = (((&kernelContext_0)->metallicTexture_0).read(vec<uint,2>(((_S27)).xy), uint(((_S27)).z)));
            break;
        }
    ...
    {
        _S39 = - _S1.eyePosition_0 * float3(rsqrt(lengthSquared_0)) ;
    }
    else
    {
        _S39 = float3(0.0f, 0.0f, 1.0f);
    }
    float3 normal_2;
    if(isFrontFace_0)
    {
    ...
    float _S40 = saturate(abs(dot(normal_2, _S39)) + 0.00000999999974738f);
    ...
    float3 diffuse_0 = diffuseColor_0 / _S46;
    ...
    else
    {
        ...
        float3 specularTint_0 = mix(float3(1.0f, 1.0f, 1.0f), diffuseColor_0, _S48);
    ...
    float3 _S54 = normalize((((float4(_S39, 0.0f)) * (_S53))).xyz);
    float3 _S55 = (((float4(_S1.eyePosition_0, 1.0f)) * (_S53))).xyz;
    float4 _S56 = float4(surface_0.lightColorAmbient_0) ;
    float3 color_0 = diffuseColor_0 * float3(_S56.w)  + diffuseColor_0 * _S49.xyz;
    ...
    for(;;)
    {
        ...
        if(_S60)
        {
            _S61 = (float4(surface_0.lightDirectionIntensity_0) ).w;
        }
        else
        {
            _S61 = (float4((&_S4->lightColorIntensity_0)->data_1[lightIndex_0]) ).w;
        }
        bool _S62;
        if(_S58)
        {
        ...
        for(;;)
        {
            ...
            float sampleIntensity_0 = _S61 / sampleCount_0;
            ...
            float3 directDiffuse_0 = diffuse_1 * (_S22 - _S78);
            ...
            float3 _S84 = diffuseColor_0 * float3(sampleIntensity_1) ;
            color_2 = color_2 + float3((_S9 * emissionScale_0 * normalDotLight_0))  * (directDiffuse_0 * float3(_S64)  * (_S84 * _S46) + directSpecular_0 * float3(_S66)  * _S84);
            sampleIndex_0 = sampleIndex_0 + 1U;
        }
    ...
    pixelOutput_0 _S85 = { float4(color_1, opacity_0) };
    return _S85;
}
