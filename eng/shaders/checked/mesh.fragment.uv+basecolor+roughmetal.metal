...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_roughmetal(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> roughnessMetallicTexture_1 [[texture(2)]])
{
    ...
    float _S13 = saturate(abs(dot(normal_2, diffuseColor_0)) + 0.00000999999974738f);
    ...
    float3 diffuse_0 = diffuseColor_1 / _S19;
    ...
    else
    {
        ...
        float3 specularTint_0 = mix(float3(1.0f, 1.0f, 1.0f), diffuseColor_1, _S21);
    ...
    float3 _S27 = normalize((((float4(diffuseColor_0, 0.0f)) * (_S26))).xyz);
    float3 _S28 = (((float4(_S1.eyePosition_0, 1.0f)) * (_S26))).xyz;
    float4 _S29 = float4(surface_0.lightColorAmbient_0) ;
    float3 color_0 = diffuseColor_1 * float3(_S29.w)  + diffuseColor_1 * _S22.xyz;
    ...
    for(;;)
    {
        ...
        {
            _S35 = (float4((&_S3->lightColorIntensity_0)->data_1[lightIndex_0]) ).xyz;
        }
        bool _S36;
        if(_S31)
        {
        ...
        {
            _S37 = (float4((&_S3->lightControls_0)->data_1[lightIndex_0]) ).x;
        }
        bool _S38;
        if(_S31)
        {
        ...
        {
            _S39 = (float4((&_S3->lightControls_0)->data_1[lightIndex_0]) ).y;
        }
        bool _S40;
        if(_S31)
        {
        ...
        for(;;)
        {
            ...
            if(lightType_0 >= 2.0f)
            {
                float3 toLight_0 = (float4((&_S3->lightPositionType_0)->data_1[lightIndex_0]) ).xyz + sampleOffsets_0[sampleIndex_0] - _S28;
            ...
            float3 _S57 = _S35 * float3(sampleIntensity_1) ;
            color_2 = color_2 + float3((_S9 * emissionScale_0 * normalDotLight_0))  * (directDiffuse_0 * float3(_S37)  * (_S57 * _S19) + directSpecular_0 * float3(_S39)  * _S57);
            sampleIndex_0 = sampleIndex_0 + 1U;
        }
    ...
    pixelOutput_0 _S58 = { float4(color_1, opacity_1) };
    return _S58;
}
