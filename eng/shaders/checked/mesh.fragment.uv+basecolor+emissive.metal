...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_emissive(pixelInput_0 _S1 [[stage_in]], float4 position_0 [[position]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]], SLANG_ParameterGroup_SceneParameters_0 constant* SceneParameters_1 [[buffer(0)]])
{
    ...
    pixelOutput_0 _S2 = { float4(abs(_S1.normal_0) * ((baseColorTexture_1).sample((materialSampler_1), (_S1.texCoord_0))).xyz + ((emissiveTexture_1).sample((materialSampler_1), (_S1.texCoord_0))).xyz, 1.0f) * SceneParameters_1->tint_0 };
    return _S2;
}
