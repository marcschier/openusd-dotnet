...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_normal_roughmetal(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], texture2d<float, access::sample> roughnessMetallicTexture_1 [[texture(2)]])
{
    ...
    float _S14 = saturate(abs(dot(normal_2, diffuseColor_0)) + 0.00000999999974738f);
    ...
    float3 diffuse_0 = diffuseColor_1 / _S20;
    ...
    else
    {
        ...
        float3 specularTint_0 = mix(float3(1.0f, 1.0f, 1.0f), diffuseColor_1, _S22);
    ...
    float3 _S28 = normalize((((float4(diffuseColor_0, 0.0f)) * (_S27))).xyz);
    float3 _S29 = (((float4(_S1.eyePosition_0, 1.0f)) * (_S27))).xyz;
    float4 _S30 = float4(surface_0.lightColorAmbient_0) ;
    float3 color_0 = diffuseColor_1 * float3(_S30.w)  + diffuseColor_1 * _S23.xyz;
    ...
    for(;;)
    {
        ...
        {
            _S36 = (float4((&_S3->lightColorIntensity_0)->data_2[lightIndex_0]) ).xyz;
        }
        bool _S37;
        if(_S32)
        {
        ...
        {
            _S38 = (float4((&_S3->lightControls_0)->data_2[lightIndex_0]) ).x;
        }
        bool _S39;
        if(_S32)
        {
        ...
        {
            _S40 = (float4((&_S3->lightControls_0)->data_2[lightIndex_0]) ).y;
        }
        bool _S41;
        if(_S32)
        {
        ...
        for(;;)
        {
            ...
            if(lightType_0 >= 2.0f)
            {
                float3 toLight_0 = (float4((&_S3->lightPositionType_0)->data_2[lightIndex_0]) ).xyz + sampleOffsets_0[sampleIndex_0] - _S29;
            ...
            float3 directDiffuse_0 = diffuse_1 * (_S13 - _S52);
            ...
            float3 _S58 = _S36 * float3(sampleIntensity_1) ;
    ...
    pixelOutput_0 _S59 = { float4(color_1, opacity_1) };
    return _S59;
}
