...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_normal_roughmetal(pixelInput_0 _S1 [[stage_in]], float4 position_0 [[position]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], texture2d<float, access::sample> roughnessMetallicTexture_1 [[texture(2)]], SLANG_ParameterGroup_SceneParameters_0 constant* SceneParameters_1 [[buffer(0)]])
{
    ...
    pixelOutput_0 _S2 = { float4(mix(abs(_S1.normal_0) * ((baseColorTexture_1).sample((materialSampler_1), (_S1.texCoord_0))).xyz, abs(((normalTexture_1).sample((materialSampler_1), (_S1.texCoord_0))).xyz * float3(2.0f)  - float3(1.0f) ), float3(0.25f) ) * float3(mix(0.5f, 1.0f, ((roughnessMetallicTexture_1).sample((materialSampler_1), (_S1.texCoord_0))).y)) , 1.0f) * SceneParameters_1->tint_0 };
    return _S2;
}
