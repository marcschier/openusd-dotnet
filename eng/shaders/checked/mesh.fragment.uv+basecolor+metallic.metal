...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_metallic(pixelInput_0 _S1 [[stage_in]], bool isFrontFace_0 [[front_facing]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], FrameParameters_natural_0 device* frameParameters_1 [[buffer(8)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler baseColorSampler_1 [[sampler(0)]], texture2d<float, access::sample> metallicTexture_1 [[texture(4)]], sampler metallicSampler_1 [[sampler(5)]])
{
    uint4 _S2;
    ...
    float _S31 = saturate(abs(dot(normal_2, diffuseColor_0)) + 0.00000999999974738f);
    ...
    float3 diffuse_0 = diffuseColor_1 / _S37;
    ...
    else
    {
        ...
        float3 specularTint_0 = mix(float3(1.0f, 1.0f, 1.0f), diffuseColor_1, _S39);
    ...
    float3 _S45 = normalize((((float4(diffuseColor_0, 0.0f)) * (_S44))).xyz);
    float3 _S46 = (((float4(_S1.eyePosition_0, 1.0f)) * (_S44))).xyz;
    float4 _S47 = float4(surface_0.lightColorAmbient_0) ;
    float3 color_0 = diffuseColor_1 * float3(_S47.w)  + diffuseColor_1 * _S40.xyz;
    ...
    for(;;)
    {
        ...
        {
            _S53 = (float4((&_S4->lightColorIntensity_0)->data_1[lightIndex_0]) ).xyz;
        }
        bool _S54;
        if(_S49)
        {
        ...
        for(;;)
        {
            ...
            float3 _S75 = _S53 * float3(sampleIntensity_1) ;
            color_2 = color_2 + float3((_S10 * emissionScale_0 * normalDotLight_0))  * (directDiffuse_0 * float3(_S55)  * (_S75 * _S37) + directSpecular_0 * float3(_S57)  * _S75);
            sampleIndex_0 = sampleIndex_0 + 1U;
        }
    ...
    pixelOutput_0 _S76 = { float4(color_1, opacity_1) };
    return _S76;
}
