...
[[fragment]] pixelOutput_0 fragmentMain_uv_basecolor_normal_emissive(pixelInput_0 _S2 [[stage_in]], float4 position_0 [[position]], SurfaceParameters_natural_0 device* surfaceParameters_1 [[buffer(7)]], texture2d<float, access::sample> baseColorTexture_1 [[texture(0)]], sampler materialSampler_1 [[sampler(0)]], texture2d<float, access::sample> normalTexture_1 [[texture(1)]], texture2d<float, access::sample> emissiveTexture_1 [[texture(3)]])
{
    ...
    float metallic_0 = saturate(_S5.x);
    float roughness_0 = clamp(_S5.y, 0.00999999977648258f, 1.0f);
    ...
    float3 sampledNormal_0 = (((&kernelContext_0)->normalTexture_0).sample(((&kernelContext_0)->materialSampler_0), (_S2.texCoord_0))).xyz * float3(2.0f)  - _S6;
    float3 tangent_1 = normalize(_S2.tangent_0.xyz);
    float3 shadingNormal_0 = normalize(tangent_1 * float3(sampledNormal_0.x)  + cross(normalize(_S2.normal_0), tangent_1) * float3(_S2.tangent_0.w)  * float3(sampledNormal_0.y)  + _S2.normal_0 * float3(sampledNormal_0.z) );
    ...
    float _S10 = max(0.00100000004749745f, _S3.y);
    ...
    float3 diffuse_0 = diffuseColor_1 / _S13;
    ...
    else
    {
        ...
        float3 specularTint_0 = mix(float3(1.0f, 1.0f, 1.0f), diffuseColor_1, _S15);
    ...
    float3 specular_0 = EvaluateDirectSpecular_0(normalIncidence_0, grazingIncidence_0, _S9, fresnel_1, normalDotLight_2, normalDotEye_2, normalDotHalf_2);
    float3 diffuse_3 = diffuse_1 * (_S6 - mix(normalIncidence_0, grazingIncidence_0, float3(fresnel_1) ));
    float3 specular_1;
    if(clearcoatAmount_0 > 0.0f)
    {
        specular_1 = specular_0 + float3(clearcoatAmount_0)  * EvaluateDirectSpecular_0(float3((reflectanceRatio_0 * reflectanceRatio_0)) , float3(1.0f, 1.0f, 1.0f), _S10, fresnel_1, normalDotLight_2, normalDotEye_2, normalDotHalf_2);
    }
    else
    {
        specular_1 = specular_0;
    }
    float4 _S16 = float4(surface_0.lightColorAmbient_0) ;
    pixelOutput_0 _S17 = { float4(float3((occlusion_0 * normalDotLight_2))  * (diffuse_3 + specular_1) * (_S16.xyz * float3(_S8.w)  * _S13) + diffuseColor_1 * float3(_S16.w)  + emissiveColor_0, opacity_1) };
    return _S17;
}
