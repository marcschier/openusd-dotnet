...
try
{
    ...
    else
    {
        ...
        $stormChildLibrary =
            Join-Path $sourceShimRoot 'lib/libopenusd_storm_child.so.9.0.0'
        $stormChildSonameLink =
            Join-Path $sourceShimRoot 'lib/libopenusd_storm_child.so.9'
        ...
        & ln -s 'libopenusd_storm_child.so.9' $stormChildLink
        if ($LASTEXITCODE -ne 0)
        {
            throw 'Could not create the synthetic Storm child SONAME symlink.'
        }
        & ln -s 'libopenusd_storm_child.so.9.0.0' $stormChildSonameLink
        if ($LASTEXITCODE -ne 0)
        {
    ...
    else
    {
        ...
        if (-not $IsMacOS)
        {
            ...
            if ($LASTEXITCODE -ne 0 -or
                $stormLinkTarget -cne 'libopenusd_storm_child.so.9')
            {
                throw (
                    'The installed Storm child ABI-9 SONAME link target is incorrect: ' +
                    $stormLinkTarget)
            }
            $installedStormSonameLink = Join-Path `
                $installedShimRoot `
                'lib/libopenusd_storm_child.so.9'
            & test -L $installedStormSonameLink
            $stormSonameLinkTarget = & readlink $installedStormSonameLink
            if ($LASTEXITCODE -ne 0 -or
                $stormSonameLinkTarget -cne 'libopenusd_storm_child.so.9.0.0')
            {
                throw (
