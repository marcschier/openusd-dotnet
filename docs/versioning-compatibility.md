...
`src/OpenUsd.Interop/OpenUsdNativeMethods.g.cs`.
...
CI runs the verification form. Do not hand-edit generated declarations without changing their source
contract and regenerating them.
...
generated by `generate-interop.py`, so header, implementation, managed constants, package validation,
and tests must be updated together.
